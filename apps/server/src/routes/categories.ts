...
export const categoryRoutes = new Elysia()
    ...
    .get("/api/newspapers/:newspaper_id/categories", async ({ params }: any) => {
        ...
        return Response.json({
            data: cats.map(c => ({ id: c.id, name: c.categoryName })),
        });
    })
    ...
    .put("/api/newspapers/:newspaper_id/categories/:category_id", async ({ params, user, roles, body }: any) => {
        if (!user) return Response.json({ error: "UNAUTHORIZED" }, { status: 401 });
        if (!roles.includes("NEWSPAPER_MANAGER") && !roles.includes("SYSTEM_ADMINISTRATOR"))
            return Response.json({ error: "FORBIDDEN" }, { status: 403 });
    ...
    .delete("/api/newspapers/:newspaper_id/categories/:category_id", async ({ params, user, roles }: any) => {
        if (!user) return Response.json({ error: "UNAUTHORIZED" }, { status: 401 });
        if (!roles.includes("NEWSPAPER_MANAGER") && !roles.includes("SYSTEM_ADMINISTRATOR"))
            return Response.json({ error: "FORBIDDEN" }, { status: 403 });
